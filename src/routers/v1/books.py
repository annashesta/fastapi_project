...
# Иньекция, мы подставляем обьект типа сессии.
# Фреймворк будет сам ииницировать функцию get_async_session


# Ручка для создания записи о книге в БД. Возвращает созданную книгу.
# @books_router.post("/books/", status_code=status.HTTP_201_CREATED)
@books_router.post(
    "/", response_model=Returnedbook, status_code=status.HTTP_201_CREATED)  # Прописываем модель ответа
async def create_book(
    book: IncomingBook, 
    session: DBSession,):  # прописываем модель валидирующую входные данные
    ...
    await session.flush()
    # flush достает из оперативной памяти данные, и добавляет id
