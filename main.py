...
# Приложение это серверная часть какого-то сервиса, которое отдает/принимает данные по API endpoint
...
class BaseBook(BaseModel):# Базовая модель Pydentik, в коорой мы прописываем типы
    title: str
    author: str
    year: int 
...
# функционал, который выполняет сам фреймворк за декоратором
# Сам endpoint описывается как бычная функция
